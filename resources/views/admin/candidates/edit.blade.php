                        ...
                        <form action="{{ route('admin.candidates.update', $candidate) }}" method="POST"
                            ...
                            <div class="text-center mb-5">
                                <div class="position-relative d-inline-block">
                                    <div class="rounded-circle overflow-hidden bg-light border border-4 border-white shadow-sm"
                                        ...
                                        <img id="imagePreview"
                                            src="{{ ($candidate->photo_path && !str_contains($candidate->photo_path, 'http')) ? asset('storage/' . $candidate->photo_path) : ($candidate->photo_path ?? 'https://via.placeholder.com/150?text=No+Photo') }}"
                                ...
                                <div class="mt-2 fw-bold text-dark display-6">{{ $candidate->name }}</div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="name" value="{{ $candidate->name }}"
                                    class="form-control form-control-lg rounded-3 shadow-none border-2" required>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="description" rows="4"
                                    class="form-control rounded-3 shadow-none border-2">{{ $candidate->description }}</textarea>
                                @error('description') <small class="text-danger">{{ $message }}</small> @enderror
                            </div>

                            <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-5">
                                <a href="{{ route('admin.candidates.index') }}"
                                    class="btn btn-theme-outline btn-lg rounded-pill px-5 fw-bold text-muted">Cancel
                                    Changes</a>
                                <button type="submit"
                                    class="btn btn-theme btn-lg rounded-pill px-5 fw-bold shadow-sm">Update
                                    Candidate</button>
                            </div>
                        </form>
                    </div>
